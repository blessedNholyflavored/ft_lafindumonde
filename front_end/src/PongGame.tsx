...
export const PongGame: React.FC<{ selectedPlayer: string }> = ({ selectedPlayer }) => {
  ...
  useEffect(() => {
    ...
    socket.on('connect', () => {
      ...
      // Envoyez le nom d'utilisateur du joueur pour enregistrer le joueur sur le serveur
      socket.emit('player-info', { username: selectedPlayer });
    });
    ...
    socket.on('player-info', ({ playerId }: { playerId: number }) => {
      console.log('Reçu les informations du joueur :', playerId);
      setPlayerId(playerId);
    });

    // Écoutez les mouvements de l'opposant et effectuez les actions du jeu en conséquence
    socket.on('opponent-move', (move: string) => {
      console.log('Mouvement de l\'opposant :', move);
      // Faites ce que vous voulez avec le mouvement de l'opposant ici
    });
  ...
  const handleMove = (move: string) => {
    if (playerId) {
      // Envoyez le mouvement au serveur WebSocket pour que l'opposant puisse le recevoir
      fetch(`http://localhost:3000/pong/move/${playerId}/${move}`, {
        method: 'POST',
      });
    }
  };
