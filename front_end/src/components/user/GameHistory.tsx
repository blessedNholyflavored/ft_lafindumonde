...
export const GameHistory = (props: any) => {
	...
	interface Game {
	  ...
	  const FetchGames = async () => {
		...
		try {
			const response = await fetch(`http://${window.location.hostname}:3000/users/${userId}/games-data`, {
				method: "GET",
				credentials: 'include',
			});
			if (response.ok)
			{
				...
				while (i < updatedGameData.length)
				{
					...
					try {
						const response = await fetch(`http://${window.location.hostname}:3000/users/${updatedGameData[i].userId1}/username`, {
							method: "GET",
							credentials: 'include',
						});
						const response2 = await fetch(`http://${window.location.hostname}:3000/users/${updatedGameData[i].userId2}/username`, {
							method: "GET",
							credentials: 'include',
						});
						if (response.ok)
