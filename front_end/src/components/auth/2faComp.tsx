...
export async function twoFAEnable(navigate: any, user: any) {
  if (user.loginLoc === true)
    return window.alert(
      "Sorry :(( You can't enable 2FA with this type of account !"
    );
  if (window.confirm("Are you ready to save your unique QR code ?")) {
    try {
      const res = await api.get("/auth/2FAenable");
      return navigate(
        `/totpSave?qrCodeImg=${encodeURIComponent(res.data.code)}`
      );
    } catch (error) {
...
export async function twoFADisable(context: any) {
  try {
    ...
    if (res.data.log2FA)
      window.alert(
        "You correctly disabled the two factor authentication ! Welcome to a new unsecured world !!!!"
      );
    else
      window.alert(
        "You can't disable something that wasn't enabled, weirdo !!!"
      );
  } catch (error) {
