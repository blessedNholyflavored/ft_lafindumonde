...
export const ChatChannel = () => {
    ...
    async function fetchRoomMessageList() {
		try {
		  const response = await fetch(`http://localhost:3001/chat/recupRoomMess/${id}`, {
			method: 'GET',
		  });
          ...
          if (data.length > 0) { 
            ...
            const usernamePromises = data.map(async (message: { senderRole: string;senderId: any; senderUsername: string; }) => {
                    try {
                        ...
                        const rolesenderResponse = await fetch(`http://localhost:3001/chat/getRole/${message.senderId}/${id}`, {
                        method: 'GET',
                        });
